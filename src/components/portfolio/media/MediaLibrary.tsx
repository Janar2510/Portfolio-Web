...
export function MediaLibrary({
  ...
  const deleteMediaMutation = useMutation({
    ...
    onSuccess: (_, deletedMediaId) => {
      queryClient.invalidateQueries({ queryKey: ['portfolio-media', siteId] });
      setSelectedMediaIds(prev => prev.filter(id => id !== deletedMediaId));
    },
  });
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {filteredMedia.map((item: any) => {
              ...
              return (
                <div
                  key={item.id}
                  className={`relative group aspect-square rounded-lg border bg-muted/30 overflow-hidden cursor-pointer transition-all hover:ring-2 hover:ring-primary/50 ${isSelected ? 'ring-2 ring-primary bg-primary/5' : ''
                    }`}
                ...
                >
                  ...
                  {multiple && (
                    <div
                      className={`absolute top-2 right-2 h-5 w-5 rounded-full border-2 flex items-center justify-center transition-colors ${isSelected
