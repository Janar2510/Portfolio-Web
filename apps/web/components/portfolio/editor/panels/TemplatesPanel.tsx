...
import { useBlocksStore, useStylesStore, useEditorStore } from '@/stores/portfolio';
import { Loader2 } from 'lucide-react';
import { useAddBlock, useUpdateBlock, useReplacePageContent } from '@/hooks/portfolio/use-editor';
...
export function TemplatesPanel() {
    ...
    const handleApplyTemplate = async (templateId: string) => {
        if (!confirm('This will replace your current page content with the template. Continue?')) {
            return;
        }

        try {
            const generated = generateSiteFromTemplate(templateId);
            ...
            if (generated.styles && Object.keys(generated.styles).length > 0) {
                ...
                const newStyles = {
                    ...(currentStyles || {}),
                    colors: generated.styles.colors || currentStyles?.colors,
                    typography: generated.styles.typography || currentStyles?.typography,
                    spacing: generated.styles.spacing || currentStyles?.spacing,
                };
            ...
            await replacePageContentMutation.mutateAsync(generated.blocks.map(b => ({
        ...
        <div className="p-4 space-y-4">
            ...
            <div className="grid gap-4">
                {templates.map((template) => (
                    ...
                    >
                        <div className={`h-24 w-full bg-muted relative flex items-center justify-center overflow-hidden border-b`}>
                            {/* Visual Preview Stub */}
                            <div className="scale-65 h-full w-full opacity-80 group-hover:scale-75 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center">
                                {template.id === 'espresso' && (
                                    <div className="w-full h-full bg-[#2B211E] p-2 flex flex-col gap-2">
                                        <div className="text-[#EFEBE9] font-serif text-[8px]">The Espresso Stroll</div>
                                ...
                                {template.id === 'ava' && (
                                    <div className="w-full h-full bg-[#FFF8F5] p-2 flex flex-col justify-center text-center">
                                        <div className="text-[#1F2937] font-bold text-[10px] leading-tight mb-2">Creative copywriter</div>
                                ...
                                {template.id === 'emily' && (
                                    <div className="w-full h-full bg-[#FAFAFA] p-2 flex flex-col gap-2">
                                        ...
                                        <div className="mt-2 text-[8px] font-serif leading-tight">Creative copywriter helping your business stand out</div>
