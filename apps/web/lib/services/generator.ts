...
export function generateSiteFromTemplate(
  ...
  // Check if it's one of the file-based templates first
  const fileTemplate = fileTemplates.find(t => t.templateId === templateId);
  if (fileTemplate) {
    const portfolioTemplate =
      convertTemplateDefToPortfolioTemplate(fileTemplate);

    // Apply styles
    if (portfolioTemplate.styles_schema) {
      styles.colors = portfolioTemplate.styles_schema.color_palette;
      styles.typography = portfolioTemplate.styles_schema.typography;
    }

    // Apply blocks from the Home page schema
    const homePage = portfolioTemplate.pages_schema.find(
      (p: any) => p.is_homepage
    );
    if (homePage && homePage.blocks) {
      // We need to map the blocks from the schema to real blocks
      // The schema blocks are already formatted correctly by the converter,
      // but we might want to run them through createBlock to ensure defaults are set if missing.
      // For now, let's assume valid structure.
      homePage.blocks.forEach((b: any) => {
        blocks.push(
          createBlock(b.block_type, {
            content: b.content,
            settings: b.settings,
            layout: b.layout,
          })
        ...
        createBlock('hero', {
          content: {
            ...
            background_image_url:
              'https://images.unsplash.com/photo-1511920170033-f8396924c348?q=80&w=2787&auto=format&fit=crop',
          },
          settings: {
            height: 'medium',
            alignment: 'left',
      ...
      blocks.push(
        createBlock('hero', {
          content: {
            ...
            subheadline:
              "I'm Ava, a passionate copywriter seeking new opportunities.",
    ...
    case 'bernadette':
      ...
      blocks.push(
        createBlock('hero', {
          content: {
            headline:
              'Digital marketing manager, specializing in content marketing.',
            subheadline:
              'Bringing in traffic and building a following is my speciality.',
      ...
      blocks.push(
        createBlock('hero', {
          content: {
            ...
            subheadline:
              "Hi there, my name is Emily. I'm a copywriter with a degree in English Literature.",
    ...
    case 'bento':
      ...
      blocks.push(
        createBlock('image', {
          content: {
            image_url:
              'https://images.unsplash.com/photo-1600607686527-6fb886090705?q=80&w=2787&auto=format&fit=crop',
          },
      ...
      blocks.push(
        createBlock('image', {
          content: {
            image_url:
              'https://images.unsplash.com/photo-1600508774662-721446edbf9c?q=80&w=2787&auto=format&fit=crop',
          },
          settings: { rounded: true },
          layout: { colSpan: 6 },
        })
