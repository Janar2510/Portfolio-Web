...
export class MicrosoftGraphProvider implements EmailProvider {
  ...
  async syncEmails(options?: SyncOptions): Promise<Email[]> {
    ...
    if (this.credentials.expires_at && this.credentials.expires_at < Date.now()) {
  ...
  private async getUserProfile(): Promise<{ mail: string; userPrincipalName: string }> {
    if (!this.credentials) {
      throw new Error('Provider not initialized');
    }
  ...
  private async fetchMessages(options?: SyncOptions): Promise<MicrosoftGraphMessage[]> {
    if (!this.credentials) {
      throw new Error('Provider not initialized');
    }
    ...
    if (options?.folder) {
      url = url.replace('/me/messages', `/me/mailFolders/${options.folder}/messages`);
    }
