...
export function createClient() {
  ...
  if (!supabaseClient) {
    ...
    if (!hasUrl || !hasKey) {
      console.error('[ERROR] Supabase environment variables missing in browser client', { hasUrl, hasKey });
    ...
    if (anonKeyLength < 100) {
      console.error('[ERROR] Supabase anon key appears to be invalid or truncated', { 
