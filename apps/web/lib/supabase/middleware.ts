...
export async function updateSession(request: NextRequest) {
  ...
  if (authCookies.length > 0) {
    ...
    console.log('[Middleware] No auth cookies found. Total cookies:', allCookies.length);
  }
  ...
  try {
    ...
    if (authCookies.length > 0 && (!user || !session)) {
      console.log('[Middleware] ⚠️ Cookie exists but no session - attempting manual recovery...');
      ...
      if (authCookie?.value) {
        try {
          ...
          console.log('[Middleware] Parsed cookie data keys:', Object.keys(cookieData));
          
          if (cookieData.access_token && cookieData.refresh_token) {
            console.log('[Middleware] Found tokens in cookie, setting session manually...');
            ...
            if (manualSessionResult.data?.session) {
              ...
              user = session.user;
              console.log('[Middleware] ✅ Manual session recovery successful! User:', user.id);
              // The setSession() call should have already updated cookies via setAll()
              // But let's verify by checking if cookies were set
              console.log('[Middleware] Session recovered, cookies should be updated by Supabase');
            } else {
              console.log('[Middleware] ❌ Manual session recovery failed:', manualSessionResult.error?.message);
            }
          } else {
            console.log('[Middleware] Cookie missing access_token or refresh_token');
          }
        } catch (parseError) {
          console.error('[Middleware] Error parsing cookie:', parseError);
          if (parseError instanceof Error) {
            console.error('[Middleware] Parse error message:', parseError.message);
          }
        }
      } else {
    ...
    if (!user) {
      console.log('[Middleware] Still no user after recovery, trying getUser()...');
      ...
      if (userResult.data?.user) {
        ...
        console.log('[Middleware] getUser() also failed:', userResult.error.message);
      }
    }
  } catch (err) {
  ...
  if (!user && !isPublicAuthRoute) {
    ...
    console.log('[Middleware] No user found, redirecting to sign-in. Path:', pathWithoutLocale);
