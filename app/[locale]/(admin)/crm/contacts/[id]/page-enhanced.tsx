...
import type { Person, Label, Note, Activity } from '@/domain/crm/types';
...
export default function PersonDetailPageEnhanced() {
  ...
  const primaryEmail = person.emails.find((e: { value: string; label: string; primary: boolean }) => e.primary) || person.emails[0];
  const primaryPhone = person.phones.find((p: { value: string; label: string; primary: boolean }) => p.primary) || person.phones[0];
          ...
          <ActivityTimeline
            ...
            onActivityUpdate={async (id, updates) => {
              await crmService.updateActivity(id, {
                subject: updates.title || undefined,
                note: updates.description || undefined,
