...
export default function DocumentsPage({ params: { locale } }: { params: { locale: string } }) {
    ...
    const { data: documents = [], isLoading } = useQuery({
        ...
        queryFn: () => userId ? documentsService.list(userId) : [],
        enabled: !!userId,
    });

    const createMutation = useMutation({
        mutationFn: () => documentsService.create({ title: 'Untitled Document', content: {} }),
        onSuccess: (doc) => {
    ...
    return (
        <div className="flex flex-col h-full bg-zinc-950">
            ...
            <div className="flex items-center justify-between p-6 border-b border-white/5">
                <div>
                    <h1 className="text-2xl font-bold text-white font-display uppercase tracking-tight">Documents</h1>
                    <p className="text-white/40">Manage your notes and documentation.</p>
                </div>
                <Button
            ...
            <div className="p-6">
                {isLoading ? (
                    <div className="text-white/20">Loading documents...</div>
                ) : documents.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-[400px] border border-dashed border-white/10 rounded-2xl">
                        ...
                        <p className="text-white/40 mb-6">Create your first document to get started.</p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {documents.map((doc) => (
                            <div key={doc.id} className="group relative bg-white/[0.02] border border-white/5 rounded-2xl p-4 hover:border-primary/20 transition-all cursor-pointer">
